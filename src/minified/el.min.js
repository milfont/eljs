...
(function(c){var f=c("<div>"),a={},k=/\$\{(\s*?)\partial\(([^}]+)\)\}/g,g=function(b){b="div#"+b.replace(/\"/g,"").replace(/\./g,"\\.").toString();return f.find(b).html().toString()},d=function(b){return a[b]||a["'"+b+"'"]||a['"'+b+'"']};c.extend({getTemplates:function(){return a},clearTemplates:function(){f.html("");a={}},compileTemplates:function(b,i){var j=b.helpers||{};j.partial=function(e){for(var h=a[b.sprite].json,c="",f=d(e),h=h.trying(e),i=0;i<h.length;i++){var g=e.split(".").last().replace(/s$/i,
"").toLowerCase(),j={};j[g]=h[i];c+=f.parse(j)}return c};c.get(b.url,function(e){f.append(e);if(e=e.match(k))for(var c=0;c<e.length;c++){var d=e[c].replace(k,"$2"),l=g(d);a[d]=(new Eljs({template:l})).compile()}a[b.sprite]=(new Eljs({template:g(b.sprite),helpers:j})).compile();typeof i==="function"&&i(a[b.sprite])})}});c.fn.extend({render:function(b,a,f){var e=c(this),h=function(a){e.append(a.parse(b))},g=d(a);g?h(g):c.compileTemplates({url:f,sprite:a},h);return e}})})(jQuery);